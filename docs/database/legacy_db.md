...
The project has an additional database called `legacy` defined in the project settings (see `config.settings.base`). Configuration is done via `.env` with the environment variables prefixed with `LEGACY_DATABASE_`. Please see the [db-management](https://github.com/opalmedapps/opal-db-management/) project for how to run the legacy database.
