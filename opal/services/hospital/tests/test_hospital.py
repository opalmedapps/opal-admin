...
def test_find_patient_by_mrn_success(mocker: MockerFixture) -> None:
    """Ensure that find_patient_by_mrn return the expected source system data structure."""
...
def test_find_by_mrn_empty_value_in_response(mocker: MockerFixture) -> None:
    ...
    source_system_patient_data_copy = SOURCE_SYSTEM_PATIENT_DATA.copy()
    source_system_patient_data_copy.update(deathDateTime='', ramqExpiration='')
    # mock find_patient_by_mrn and pretend it was successful
    RequestMockerTest.mock_requests_post(
        mocker,
        {
            'status': 'success',
            'data': source_system_patient_data_copy,
        },
    )
...
def test_find_by_ramq_patient_not_found(mocker: MockerFixture) -> None:
    ...
    response = source_system_service.find_patient_by_ramq('SIMM86100199')
    assert response['status'] == 'error'
    assert response['data']['message'] == ['not_found']
...
def test_find_patient_by_mrn_invalid_mrn() -> None:
    ...
    response = source_system_service.find_patient_by_mrn('', SITE_CODE)
    assert response['status'] == 'error'
...
def test_find_patient_by_mrn_invalid_site() -> None:
    ...
    response = source_system_service.find_patient_by_mrn(MRN, '')
    assert response['status'] == 'error'
    assert response['data']['message'] == 'Provided MRN or site is invalid.'


def test_find_patient_by_ramq_success(mocker: MockerFixture) -> None:
    """Ensure that find_patient_by_ramq return the expected source system data structure."""
...
def test_find_patient_by_ramq_invalid_ramq(mocker: MockerFixture) -> None:
    ...
    response = source_system_service.find_patient_by_ramq(RAMQ_INVALID)
    assert response['status'] == 'error'
...
def test_new_opal_patient_success(mocker: MockerFixture) -> None:
    ...
    response = source_system_service.new_opal_patient(
        [
            ('RVH', '0000001'),
            ('MCH', '0000002'),
        ],
