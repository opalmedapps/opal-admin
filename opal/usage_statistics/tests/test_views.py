...
test_url_template_data: list[tuple[str, str]] = [
    (reverse('usage-statistics:data-export'), 'usage_statistics/export_data/export_form.html'),
]
...
def test_usage_statistics_export_unauthorized(user_client: Client) -> None:
    """Ensure that an authenticated (not admin) user cannot access the Export Data page."""
    response = user_client.get(reverse('usage-statistics:data-export'))
