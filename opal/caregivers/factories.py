...
class DeviceProvider(BaseProvider):
    ...
    def device_id(self) -> str:
        ...
        Returns:
            A random device_id value.
        """
        length = random.randint(16, 100)  # Choose a reasonable device_id length
        char_choices = [str(digit) for digit in range(9)] + ['a', 'b', 'c', 'd', 'e', 'f']  # 0-9 digits and letters up to 'f' # noqa: WPS221, E501
        chars = [random.choice(char_choices) for _ in range(length)]  # Make random choices to build up the string
        return ''.join(chars)
