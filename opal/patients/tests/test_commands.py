...
class TestExpireRelationshipsCommand(CommandTestMixin):
    ...
    @patch.object(Patient, 'calculate_age', side_effect=calculate_age_fixed_date)
    def test_no_end_age(self, mocker: MockerFixture):
        """Test a relationship with no end age, which shouldn't be affected"""
        relationship = self.create_relationship(date(1900, 1, 1), end_age=None)
        self._call_command('expire_relationships')
        relationship.refresh_from_db()
        assert relationship.status == RelationshipStatus.CONFIRMED.value

    @patch.object(Patient, 'calculate_age', side_effect=calculate_age_fixed_date)
    def test_pending_unaffected(self, mocker: MockerFixture):
        """Test a relationship with pending status, which shouldn't be affected"""
        relationship = self.create_relationship(date(1900, 1, 1), status=RelationshipStatus.PENDING)
        self._call_command('expire_relationships')
        relationship.refresh_from_db()
        assert relationship.status == RelationshipStatus.PENDING.value

    @patch.object(Patient, 'calculate_age', side_effect=calculate_age_fixed_date)
    def test_denied_unaffected(self, mocker: MockerFixture):
        """Test a relationship with denied status, which shouldn't be affected"""
        relationship = self.create_relationship(date(1900, 1, 1), status=RelationshipStatus.DENIED)
        self._call_command('expire_relationships')
        relationship.refresh_from_db()
        assert relationship.status == RelationshipStatus.DENIED.value

    @patch.object(Patient, 'calculate_age', side_effect=calculate_age_fixed_date)
    def test_expired_unaffected(self, mocker: MockerFixture):
        """Test a relationship with already expired status, which shouldn't be affected"""
        relationship = self.create_relationship(date(1900, 1, 1), status=RelationshipStatus.EXPIRED)
        self._call_command('expire_relationships')
        relationship.refresh_from_db()
        assert relationship.status == RelationshipStatus.EXPIRED.value

    @patch.object(Patient, 'calculate_age', side_effect=calculate_age_fixed_date)
    def test_revoked_unaffected(self, mocker: MockerFixture):
        """Test a relationship with revoked status, which shouldn't be affected"""
        relationship = self.create_relationship(date(1900, 1, 1), status=RelationshipStatus.REVOKED)
        self._call_command('expire_relationships')
        relationship.refresh_from_db()
        assert relationship.status == RelationshipStatus.REVOKED.value

    def create_relationship(self, patient_date_of_birth, end_age=14, status=RelationshipStatus.CONFIRMED):
        """Helper method to quickly create a relationship with a patient who has a specific birthday."""
